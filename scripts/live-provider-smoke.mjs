...
const started=Date.now();const card=await generateSuggestion(provider,config,request);console.log(JSON.stringify({provider,elapsedMs:Date.now()-started,card},null,2));assert.equal(card.detectedLanguage,"EN");assert.ok(card.replies.length>=1);assert.ok(!/^(user|they|speaker) said:/i.test(card.englishContext));for(const reply of card.replies){assert.ok(reply.english.length<=60,`Branch too long: ${reply.english}`);assert.match(reply.english,/[.!?]$/);}
