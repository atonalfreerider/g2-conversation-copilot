...
final class OutputSanitizer {
    static String cleanTranslation(String value){return value.trim().replaceFirst("(?i)^(?:user|they|speaker)\\s+(?:said|says)\\s*:\\s*","").replaceFirst("(?i)\\s*[-–—,:;]*\\s*(?:a\\s+)?(?:polish|russian|ukrainian|greek|chinese)\\s+(?:friendly\\s+)?greeting\\.?$","").trim();}
