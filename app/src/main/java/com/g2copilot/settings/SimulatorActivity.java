...
public final class SimulatorActivity extends Activity {
    ...
    private int focus=0, viewport=0, risk=4; private boolean playful=true, inquisitive=true, listening=true;
    ...
    @Override public void onCreate(Bundle state){super.onCreate(state);settings=new SecureSettings(this);String bio=getSharedPreferences("profile",MODE_PRIVATE).getString("biography","");memory=new ConversationMemory(this,bio);tts=new TextToSpeech(this,status->{if(status==TextToSpeech.SUCCESS)tts.setLanguage(Locale.US);});setContentView(build());updateTranscript();render();}
    ...
    private void commitSpeech(String heard,boolean wearer){
        ...
        updateContext(heard,wearer);render();if(wearer&&!"EN".equals(languageCode())&&!"AUTO".equals(languageCode()))requestSuggestions(rollingText(""),heard);else requestSuggestions(rollingText(""),"");lastSuggestedSpeech=heard;
    }
    ...
    private void requestSuggestions(String heard){requestSuggestions(heard,"");}
    private void requestSuggestions(String heard,String instant){
        ...
        String biography=getSharedPreferences("profile",MODE_PRIVATE).getString("biography","");final String prompt=PromptBuilder.build(languageCode(),playful,inquisitive,risk,biography,memory.compressed()+"\nLive: "+heard,instant);final boolean pin=!instant.trim().isEmpty();
    ...
    private void setSpeechLanguages(List<String> tags){String selected=languageTag();LinkedHashSet<String> filtered=new LinkedHashSet<>();filtered.add("en-US");for(String tag:tags){Locale locale=Locale.forLanguageTag(tag);if(!"en".equalsIgnoreCase(locale.getLanguage()))filtered.add(tag);}speechTags.clear();speechTags.addAll(filtered);List<String> labels=new ArrayList<>();labels.add("Auto (phone default)");for(String tag:speechTags){Locale locale=Locale.forLanguageTag(tag);labels.add(locale.getDisplayName()+" — "+tag);}speechLanguage.setAdapter(new ArrayAdapter<>(this,android.R.layout.simple_spinner_dropdown_item,labels));if(selected!=null){int index=speechTags.indexOf(selected);if(index>=0)speechLanguage.setSelection(index+1);}}
    ...
    private void moveFocus(int direction){int max=2+branches.size();focus=Math.max(0,Math.min(max,focus+direction));if(focus>=3){int selected=focus-3;if(selected<viewport)viewport=selected;if(selected>viewport+1)viewport=selected-1;viewport=Math.max(0,Math.min(Math.max(0,branches.size()-2),viewport));}render();}
    private void activateFocus(){if(focus==0)playful=!playful;else if(focus==1)inquisitive=!inquisitive;else if(focus==2)risk=risk==7?1:risk+1;else{int selected=focus-3;if(selected<branches.size()&&tts!=null)tts.speak(branches.get(selected).speech(!"EN".equals(language)),TextToSpeech.QUEUE_FLUSH,null,"branch-"+selected);return;}String heard=rollingText(pendingSpeech);if(!heard.isEmpty())updateSuggestions(heard,false);else render();}
    private void replaceQueue(List<Branch> next){branches.clear();branches.addAll(next.subList(0,Math.min(6,next.size())));viewport=0;if(focus>2)focus=3;}
    private void pinBranch(Branch branch){branches.removeIf(b->b.english.equals(branch.english));branches.add(0,branch);while(branches.size()>6)branches.remove(branches.size()-1);viewport=0;focus=3;}
    private void submitInstantTranslation(){String phrase=instantPhrase.getText().toString().trim();if(phrase.isEmpty())return;memory.append(true,phrase);updateTranscript();instantPhrase.setText("");if("EN".equals(languageCode())||"AUTO".equals(languageCode()))pinBranch(new Branch(phrase,""));else requestSuggestions(rollingText(""),phrase);render();}
    ...
    private void render(){
        String p=playful?"P":"S",s=inquisitive?"I":"D",r=String.valueOf(risk);if(focus==0)p="["+p+"]";if(focus==1)s="["+s+"]";if(focus==2)r="["+r+"]";
        String status=language+" "+p+" "+s+" "+r;contextLine.setText(context);branchOne.setText(renderBranch(viewport,status));branchTwo.setText(renderBranch(viewport+1,""));String target=focus<3?axes[focus]:"branch "+(focus-2)+" (press to speak)";axisHint.setText((microphoneActive?"Listening · ":"")+"Selected: "+target+" · scroll selects, press acts");
    }
    private String renderBranch(int index,String status){if(index>=branches.size())return index==0?(listening?status:"PAUSED "+status)+" • Waiting for a complete suggestion…":"";String marker=focus==index+3?">":"•";String prefix=status.isEmpty()?marker:(listening?status:"PAUSED "+status)+" "+marker;return prefix+" "+branches.get(index).display(!"EN".equals(language));}
