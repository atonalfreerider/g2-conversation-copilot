...
public final class SimulatorActivity extends Activity {
    ...
    private int focus=0, viewport=0; private boolean playful=true, listening=true;
    ...
    private void requestSuggestions(String heard,String instant){
        if(heard.trim().isEmpty())return;long generation=++desiredGeneration;if(requestInFlight){queuedRefresh=true;if(!instant.trim().isEmpty())queuedInstant=instant;java.net.HttpURLConnection c=activeConnection;if(c!=null)c.disconnect();return;}requestInFlight=true;lastSuggestedSpeech=pendingSpeech.isEmpty()?heard:pendingSpeech;Provider provider=settings.activeProvider();
    ...
    private void runQueuedRefresh(){if(queuedRefresh||(!pendingSpeech.isEmpty()&&!pendingSpeech.equals(lastSuggestedSpeech))){queuedRefresh=false;String latest=rollingText(pendingSpeech);String instant=queuedInstant;queuedInstant="";if(!latest.isEmpty())requestSuggestions(latest,instant);}}
    ...
    private void activateFocus(){if(focus<branches.size()&&tts!=null)tts.speak(branches.get(focus).speech(!"EN".equals(language)),TextToSpeech.QUEUE_FLUSH,null,"branch-"+focus);}
    ...
    private void render(){
        contextLine.setText(context);branchOne.setText(renderBranch(viewport));branchTwo.setText(renderBranch(viewport+1));axisHint.setText((microphoneActive?"Listening · ":"")+"Response "+(branches.isEmpty()?0:focus+1)+" of "+branches.size()+" · scroll selects, press speaks");
    }
    private String renderBranch(int index){if(index>=branches.size())return index==0?"Waiting for complete suggestions…":"";return (focus==index?"> ":"• ")+branches.get(index).display(!"EN".equals(language));}
