...
final class OnDeviceGeminiClient {
    static ProviderClient.Suggestions suggest(String prompt,ProviderClient.LogSink log) throws Exception {
        ...
        String raw=response.getCandidates().get(0).getText().trim().replaceFirst("^```(?:json)?\\s*","").replaceFirst("\\s*```$","");JSONObject result=new JSONObject(raw);String detected=result.optString("language","EN").toUpperCase();JSONArray values=result.getJSONArray("branches");java.util.List<Branch> branches=new java.util.ArrayList<>();for(int i=0;i<Math.min(6,values.length());i++){JSONObject branch=values.optJSONObject(i);if(branch==null)continue;String english=branch.optString("english","").replaceAll("\\s+"," ").trim();String phonetic=OutputSanitizer.cleanPhonetic(branch.optString("phonetic","")).replaceAll("\\s+"," ").trim();if(!english.isEmpty()&&("EN".equals(detected)||!phonetic.isEmpty()))branches.add(new Branch(english,phonetic));}if(branches.isEmpty())throw new IllegalStateException("Gemini Nano returned no complete branches");while(branches.size()<6)branches.add(branches.get(branches.size()%Math.max(1,branches.size())));
