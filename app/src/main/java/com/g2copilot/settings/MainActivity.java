...
public final class MainActivity extends Activity {
    ...
    private View buildUi() {
        ...
        nanoStatus=text("Live captions use the saved OpenAI key; Pixel speech is the offline fallback.",14);root.addView(nanoStatus);nanoMode=button("Bring Gemini Nano to foreground");root.addView(nanoMode);
        root.addView(text("Foreign translation stays on-device with Google ML Kit. Branch generation uses the backend selected below.",12));
        ...
        root.addView(label("Conversation style")); persona=choice("Playful","Strategic / trust-building","Argumentative / contrarian");persona.setSelection("S".equals(getPreferences(MODE_PRIVATE).getString("persona","P"))?1:"C".equals(getPreferences(MODE_PRIVATE).getString("persona","P"))?2:0); root.addView(persona);
        ...
        save.setOnClickListener(v->saveConnection()); test.setOnClickListener(v->testConnection()); saveDefaults.setOnClickListener(v->{String[] personas={"P","S","C"};getPreferences(MODE_PRIVATE).edit().putInt("words",number(words,20)).putString("persona",personas[Math.min(persona.getSelectedItemPosition(),2)]).apply();getSharedPreferences("profile",MODE_PRIVATE).edit().putString("biography",biography.getText().toString().trim()).putBoolean("native_characters",nativeCharacters.isChecked()).apply();show("Defaults and biography saved on phone");});
        nanoMode.setOnClickListener(v->{getWindow().addFlags(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON);startForegroundService(new Intent(this,CompanionBridgeService.class).setAction(CompanionBridgeService.ACTION_NANO_FOREGROUND));show("Gemini Nano active. Keep this screen visible while Nano generates branches.");applyProviderMode(selected());});
        return scroll;
    }
    private void applyProviderMode(Provider chosen){boolean cloud=chosen!=Provider.GEMINI_NANO;apiKey.setEnabled(cloud);model.setEnabled(cloud);endpoint.setEnabled(cloud);test.setText(cloud?"Test connection":"Check Gemini Nano");if(chosen==Provider.GEMINI_NANO){getWindow().addFlags(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON);nanoStatus.setText(ForegroundState.mainActivityVisible?"Pixel speech background-ready · Gemini Nano active":"Pixel speech background-ready · Nano needs this screen foreground");nanoMode.setEnabled(true);}else{getWindow().clearFlags(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON);nanoStatus.setText("Pixel speech + translation run locally in background · branches: "+chosen.label);nanoMode.setEnabled(false);}nanoMode.setText("Bring Gemini Nano to foreground");}
    ...
    @Override protected void onResume(){super.onResume();ForegroundState.mainActivityVisible=true;if(settings!=null)applyProviderMode(settings.activeProvider());startForegroundService(new Intent(this,CompanionBridgeService.class).setAction(CompanionBridgeService.ACTION_NANO_FOREGROUND));}
    @Override protected void onPause(){ForegroundState.mainActivityVisible=false;if(nanoStatus!=null)nanoStatus.setText(settings!=null&&settings.activeProvider()==Provider.GEMINI_NANO?"Pixel speech background-ready · Nano paused":"Pixel speech + translation continue in background");super.onPause();}
