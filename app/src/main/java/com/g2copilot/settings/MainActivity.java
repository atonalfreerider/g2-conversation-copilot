...
public final class MainActivity extends Activity {
    ...
    private View buildUi() {
        ...
        nanoStatus=text("On-device speech is active while this companion stays foregrounded.",14);root.addView(nanoStatus);nanoMode=button("Enable on-device speech foreground");root.addView(nanoMode);
        root.addView(text("On-device transcription and translations by Google ML Kit.",12));
        ...
        nanoMode.setOnClickListener(v->{getWindow().addFlags(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON);startForegroundService(new Intent(this,CompanionBridgeService.class).setAction(CompanionBridgeService.ACTION_NANO_FOREGROUND));show("On-device speech foreground active. Keep this screen visible; the G2 plugin can remain running behind it.");applyProviderMode(selected());});
        return scroll;
    }
    private void applyProviderMode(Provider chosen){boolean cloud=chosen!=Provider.GEMINI_NANO;apiKey.setEnabled(cloud);model.setEnabled(cloud);endpoint.setEnabled(cloud);test.setText(cloud?"Test connection":"Check Gemini Nano");getWindow().addFlags(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON);if(chosen==Provider.GEMINI_NANO)nanoStatus.setText(ForegroundState.mainActivityVisible?"On-device speech + Nano active · screen kept awake":"Speech and Nano paused · bring this screen forward");else nanoStatus.setText(ForegroundState.mainActivityVisible?"Pixel on-device speech active · selected branch provider: "+chosen.label:"On-device speech paused · bring this screen forward");nanoMode.setText("Keep on-device speech foreground");}
    ...
    @Override protected void onResume(){super.onResume();ForegroundState.mainActivityVisible=true;getWindow().addFlags(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON);if(settings!=null)applyProviderMode(settings.activeProvider());startForegroundService(new Intent(this,CompanionBridgeService.class).setAction(CompanionBridgeService.ACTION_NANO_FOREGROUND));}
    @Override protected void onPause(){ForegroundState.mainActivityVisible=false;if(nanoStatus!=null)nanoStatus.setText("On-device speech paused until this companion returns to the foreground");super.onPause();}
