...
public final class MainActivity extends Activity {
    ...
    private TextView status; private Button test;
    ...
    private View buildUi() {
        ...
        save.setOnClickListener(v->saveConnection()); test.setOnClickListener(v->testConnection()); saveDefaults.setOnClickListener(v->{getPreferences(MODE_PRIVATE).edit().putInt("words",number(words,20)).putString("persona",persona.getSelectedItemPosition()==0?"P":"S").apply();getSharedPreferences("profile",MODE_PRIVATE).edit().putString("biography",biography.getText().toString().trim()).apply();show("Defaults and biography saved on phone");});
    ...
    private void testConnection() { saveConnection(); test.setEnabled(false); show("Testing " + selected().label + "…"); network.execute(()->{ try { String result;if(selected()==Provider.GEMINI_NANO){OnDeviceGeminiClient.suggest("Return only JSON with language EN, context Gemini Nano ready, and eight branch objects containing english and empty phonetic fields.",null);result="Gemini Nano ready · no API key";}else result=ProviderClient.test(selected(),endpoint.getText().toString().trim(),model.getText().toString().trim(),apiKey.getText().toString().trim());runOnUiThread(()->{show(result);test.setEnabled(true);}); } catch(Exception e){runOnUiThread(()->{show(e.getMessage());test.setEnabled(true);});} }); }
