...
final class ProviderClient {
    ...
    static Suggestions suggest(Provider provider,String baseUrl,String model,String apiKey,String prompt,LogSink log,Cancellation cancellation) throws Exception {
        ...
        String detected=result.optString("language","EN").toUpperCase();JSONArray values=result.getJSONArray("branches");if(values.length()<1)throw new IllegalStateException("Provider returned no branches");java.util.List<Branch> branches=new java.util.ArrayList<>();for(int i=0;i<Math.min(8,values.length());i++){JSONObject branch=values.optJSONObject(i);if(branch==null)throw new IllegalStateException("Provider returned legacy phonetic-only branch");String english=oneLine(branch.optString("english",""));String phonetic=oneLine(OutputSanitizer.cleanPhonetic(branch.optString("phonetic","")));if(english.isEmpty())throw new IllegalStateException("English branch is required");if(!"EN".equals(detected)&&phonetic.isEmpty())throw new IllegalStateException("Foreign branch requires both English and phonetic");if(log!=null&&(english.length()>72||phonetic.length()>72))log.log("WARN branch exceeded two-line G2 target");branches.add(new Branch(english,phonetic));}int seed=branches.size();for(int i=0;branches.size()<8;i++)branches.add(branches.get(i%seed));return new Suggestions(detected,OutputSanitizer.cleanTranslation(result.optString("context","")),branches);
    }
