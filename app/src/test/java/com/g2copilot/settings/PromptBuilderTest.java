...
public final class PromptBuilderTest {
    ...
    @Test public void foreignBranchContractHasThreeDistinctFields(){String p=PromptBuilder.build("PL",true,true,4,"Traveler","Dzień dobry");assertTrue(p.contains("{english, native, phonetic}"));assertTrue(p.contains("english is meaning only"));assertTrue(p.contains("Phonetic must never pronounce the English meaning"));assertTrue(p.contains("exactly EIGHT"));}
