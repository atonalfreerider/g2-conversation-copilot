...
const BUILD='0.3.2-20260915',AUDIO_FRAME_BYTES=3200
...
async function render(){if(!bridge)return;if(rendering){renderQueued=true;return}rendering=true;do{renderQueued=false;const nextContext=context,nextBranches=branchContent(),tasks:Promise<any>[]=[];if(nextContext!==renderedContext){renderedContext=nextContext;tasks.push(bridge.textContainerUpgrade({containerID:1,containerName:'context',content:nextContext}))}if(nextBranches!==renderedBranches){renderedBranches=nextBranches;tasks.push(bridge.textContainerUpgrade({containerID:4,containerName:'branches',content:nextBranches}))}if(tasks.length)await Promise.all(tasks)}while(renderQueued||context!==renderedContext||branchContent()!==renderedBranches);rendering=false}
...
async function start(){
  ...
  const page={containerTotalNum:2,textObject:[{xPosition:0,yPosition:0,width:576,height:72,paddingLength:0,containerID:1,containerName:'context',content:context,isEventCapture:0},{xPosition:0,yPosition:72,width:576,height:216,paddingLength:0,containerID:4,containerName:'branches',content:branchContent(),isEventCapture:1}],menuObject:{menuItems:[{itemName:'Refresh branches',itemID:1},{itemName:'Start / stop mic',itemID:2}]}}
